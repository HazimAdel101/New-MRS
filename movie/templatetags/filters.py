...
@register.filter(name='star_evaluation')
def star_evaluation(value, arg):
    try:
        rating = float(value)
        i = int(arg)
    except (TypeError, ValueError):
        # Handle the case where either value or arg cannot be converted to float or int
        return '#'

    difference = rating - i + 1
    if difference >= 1:
